...
⚠️ **重要**: 静的サイトは**Cloudflare Pages**を使用してください。**Cloudflare Worker**ではありません。
...
2. **Cloudflare Pagesでプロジェクトを作成**
   ...
   - **「Pages」タブ**を選択（「Workers」タブではありません）
...
このエラーは、**Cloudflare Worker**の設定画面で作業している場合に発生します。静的サイトには**Cloudflare Pages**を使用してください。

**解決方法：**

1. 左サイドバーから **「Workers & Pages」** をクリック
2. **「Pages」タブ**を選択（「Workers」タブではないことを確認）
3. 既存のプロジェクトがある場合は削除し、新しく作成
4. または、既存のプロジェクトの **Settings** > **Builds & deployments** を開く
5. **Build command** フィールドを**完全に空欄**にする（何も入力しない）
6. **Deploy command** フィールドも**空欄**にする
7. **Save** をクリック
8. **Deployments**タブに戻り、**Retry deployment**をクリック
